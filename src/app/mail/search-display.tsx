        ...
        <div className='p-4 max-h-[calc(100vh-50px)] overflow-y-scroll'>
            <div className='flex items-centergap-2 mb-4'>
                <h2 className='text-gray-600 text-sm dark:text-gray-400'>
                    Your Search for &quot;{searchValue}&quot; came back with...</h2>
            </div>
            {search.data?.hits.length === 0 ? (<h1> No results found</h1>) : (
                <ul className='flex flex-col gap-2'>
                    {search.data?.hits.map(hit => {
                        (
                            <li id={hit.id} className='border list-none rounded-md p-4 hover:bg-gray-100 cursor-pointer transition-all dark:hover:bg-gray-900'>
                                <h3 className='text-base font-medium'>{hit.document.subject}</h3>
                                <p className='text-sm text-gray-500'>From: {hit.document.from}</p>
                                <p className='text-sm text-gray-500'>To: {hit.document.to.join(', ')}</p>
